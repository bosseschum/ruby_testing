...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 5 and max is 9' do
      subject(:game_one) { described_class.new(5, 9, number_guessing) }
      it 'returns 7' do
        expect(game_one.make_guess).to eq(7)
      end
    end

    context 'when min is 8 and max is 9' do
      subject(:game_two) { described_class.new(8, 9, number_guessing) }
      it 'returns 8' do
        expect(game_two.make_guess).to eq(8)
      end
    end

    context 'when min is 0 and max is 3' do
      subject(:game_three) { described_class.new(0, 3, number_guessing) }
      it 'returns 1' do
        expect(game_three.make_guess).to eq(1)
      end
    end

    context 'when min and max both equal 3' do
      subject(:game_four) { described_class.new(3, 3, number_guessing) }
      it 'returns 3' do
        expect(game_four.make_guess).to eq(3)
      end
    end
  end
  ...
  describe '#game_over?' do
    let(:guess_over) { double('guess_over', value: 5) }
    context 'when guess and random_number are equal' do
      ...
      subject(:game_over_guess) { described_class.new(2, 8, guess_over, 5) }

      it 'is game over' do
        expect(game_over_guess.game_over?).to be true
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      subject(:game_not_over) { described_class.new(4, 8, guess_over) }
      it 'is not game over' do
        expect(game_not_over.game_over?).not_to be true
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:mid_guess_game) { described_class.new(5, 8, number_range, 7) }
      it 'updates min to the same value as max' do
        expect { mid_guess_game.update_range }.to change { mid_guess_game.min }.to(mid_guess_game.max)
      end

      it 'does not update max' do
        expect { mid_guess_game.update_range }.not_to(change { mid_guess_game.max })
      end
    end
  end
end
